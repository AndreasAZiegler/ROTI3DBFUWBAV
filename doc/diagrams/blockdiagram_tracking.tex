...
\begin{document}
	\begin{tikzpicture}[auto, node distance=2cm,>=latex']
		...
		\node [branch] (p3) [right = 3cm of rosbag] {};
		...
		\node [point] (state) [right = 4.5cm of fusing] {};
		
		\draw [-] (rosbag) -- node[name=u] {/camera/video} (p3);
		\draw [-] (p3) -- node[name=u,below = 1.6cm,anchor=west] {/uwb/tracker} (vision_tracker);
		...
		\draw [->] (p4) -- node[name=u] {} (p4-|fusing.west);
		
		
		\draw [->] (fusing) -- node[name=u] {/fusing/coordinates} (state);
